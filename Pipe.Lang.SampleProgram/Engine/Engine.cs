...
public PushSignalSource<int> makeWordCountSignal () => WordSignal.Filter((CompletedWord word) => word.FullContent.Length >= 5).Reduce((int count, CompletedWord word) => count + 1, 0);
...
makeWordCountSignal().ForEach((int count) => {
WordCount = count;
});
}
}